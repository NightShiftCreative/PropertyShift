jQuery(document).ready(function($) {
	...
	$('.admin-module-floor-plans').on('click', '.add-floor-plan', function() {
		...
		var floorPlanItem = '\
            <h3 class="accordion-tab"><i class="fa fa-chevron-right icon"></i> <span class="repeater-title-mirror floor-plan-title-mirror">'+ rao_real_estate_local_script.new_floor_plan +'</span> <span class="delete delete-floor-plan right"><i class="fa fa-trash"></i> '+ rao_real_estate_local_script.delete_text +'</span></h3> \
			<div class="floor-plan-item"> \
				<div class="floor-plan-left"> \
					<label>'+ rao_real_estate_local_script.floor_plan_title +' </label> <input class="repeater-title floor-plan-title" type="text" name="rypecore_floor_plans['+count+'][title]" placeholder="'+ rao_real_estate_local_script.new_floor_plan +'" /><br/> \
					<label>'+ rao_real_estate_local_script.floor_plan_size +' </label> <input type="text" name="rypecore_floor_plans['+count+'][size]" /><br/> \
					<label>'+ rao_real_estate_local_script.floor_plan_rooms +' </label> <input type="number" name="rypecore_floor_plans['+count+'][rooms]" /><br/> \
					<label>'+ rao_real_estate_local_script.floor_plan_bathrooms +' </label> <input type="number" name="rypecore_floor_plans['+count+'][baths]" /><br/> \
				</div> \
                <div class="floor-plan-right"> \
				    ...
				    <div class="floor-plan-img"> \
                        ...
                        <input id="_btn" class="rao_upload_image_button" type="button" value="'+ rao_real_estate_local_script.upload_img +'" /> \
                        <span class="button-secondary remove">'+ rao_real_estate_local_script.remove_text +'</span> \
                    </div> \
                </div> \
                <div class="clear"></div> \
			</div> \
    ...
    $('.admin-module-custom-fields').on('click', '.add-custom-field', function() {
        ...
        if(customFieldExists(fieldValue)) { 
            alert(rao_real_estate_local_script.custom_field_dup_error); 
        } else { 
            var customFieldItem = '\
                <table class="custom-field-item sortable-item"> \
                    <tr> \
                        <td> \
                            <label>'+rao_real_estate_local_script.value_text+'</label> \
                            ...
                            <div class="edit-custom-field-form hide-soft"> \
                                <table class="admin-module"> \
                                    <tr> \
                                    <td class="admin-module-label"><label>'+rao_real_estate_local_script.field_type_text+'</label></td> \
                                    <td class="admin-module-field"> \
                                        <select class="custom-field-type-select" name="rypecore_custom_fields['+count+'][type]"> \
                                            <option value="text">'+rao_real_estate_local_script.text_input_text+'</option> \
                                            <option value="num">'+rao_real_estate_local_script.num_input_text+'</option> \
                                            <option value="select">'+rao_real_estate_local_script.select_text+'</option> \
                                        </select> \
                                    </td> \
                                    </tr> \
                                </table> \
                                <table class="admin-module admin-module-select-options hide-soft"> \
                                    <tr> \
                                    <td class="admin-module-label"><label>'+rao_real_estate_local_script.select_options_text+'</label></td> \
                                    <td class="admin-module-field"> \
                                        <div class="custom-field-select-options-container"></div> \
                                        <div class="button add-custom-field-select">'+rao_real_estate_local_script.select_options_add+'</div> \
                                    </td> \
                                    </tr> \
                                </table> \
                                <table class="admin-module no-border"> \
                                    <tr> \
                                    <td class="admin-module-label"><label>'+rao_real_estate_local_script.front_end_text+'</label></td> \
                                    <td class="admin-module-field"><input type="checkbox" name="rypecore_custom_fields['+count+'][front_end]" checked /></td> \
                                    </tr> \
                                </table> \
                            </div> \
                        </td> \
                        <td class="custom-field-action edit-custom-field"><div class="sortable-item-action"><i class="fa fa-cog"></i> '+rao_real_estate_local_script.edit_text+'</div></td> \
                        <td class="custom-field-action delete-custom-field"><div class="sortable-item-action"><i class="fa fa-trash"></i> '+rao_real_estate_local_script.remove_text+'</div></td> \
                    </tr> \
                </table> \
            ';
    ...
    $('.admin-module-custom-fields').on("click", ".delete-custom-field", function() {
        var confirmMessage = rao_real_estate_local_script.delete_custom_field_confirm;
        if (confirm(confirmMessage) == true) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action : 'rao_delete_custom_field',
                    key: customFieldID
                },
    ...
    $('.admin-module-custom-fields').on("click", ".add-custom-field-select", function() {
        ...
        var selectOption = '<p><input type="text" name="rypecore_custom_fields['+count+'][select_options][]" placeholder="'+rao_real_estate_local_script.option_name_text+'" /><span class="delete-custom-field-select"><i class="fa fa-times"></i></span></p>';
            ...
            var customFilterField = '\
                <li class="sortable-item custom-filter-field custom-filter-field-'+customFilterFieldID+'"> \
                    <div class="sortable-item-header"> \
                        ...
                        <span class="sortable-item-action remove right"><i class="fa fa-times"></i> '+rao_real_estate_local_script.remove_text+'</span> \
