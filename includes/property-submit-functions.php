...
function rao_insert_property_post($edit_property_id = null) {
	...
	// require a title
	if(trim($_POST['title']) === '') {
	    $errors['title'] =  esc_html__('Please enter a title!', 'rype-add-ons'); 
	} else {
	...
	if(trim($_POST['street_address']) === '') {
	    $errors['address'] =  esc_html__('Please enter an address!', 'rype-add-ons'); 
	} else {
	...
	// require a price
	if(trim($_POST['price']) === '') {
	    $errors['price'] =  esc_html__('Please enter a price!', 'rype-add-ons'); 
	} else {
	...
	if(empty($errors)) {
		...
		if($members_submit_property_approval == 'true') {
	        $output['success'] = esc_html__('Your property,', 'rype-add-ons') .' <b>'. $title .',</b> '. esc_html__('was submitted for review!', 'rype-add-ons');
	    } else {
	        $output['success'] = esc_html__('Your property,', 'rype-add-ons') .' <b>'. $title .',</b> '. esc_html__('was published!', 'rype-add-ons');
	    }
	} else {
	    $output['success'] = '';
	}
...
function rao_property_submit_form() { 
	...
	if (isset($_GET['edit_property']) && !empty($_GET['edit_property'])) {
	    $form_submit_text = esc_html__('Update Property', 'rype-add-ons');
    	...
    	$edit_property_location = rao_get_property_location($edit_property_id , null, 'true');
    	$edit_property_amenities = rao_get_property_amenities($edit_property_id , true, 'true');
    	$edit_property_status = rao_get_property_status($edit_property_id, 'true');
    	$edit_property_type = rao_get_property_type($edit_property_id, 'true');
		...
		$edit_property_id = '';
		$form_action = '';
		$form_submit_text = esc_html__('Submit Property', 'rype-add-ons');
	}
	...
	<div class="user-submit-property-form">
		...
		<?php if ($success != '') { ?>
	        <div class="alert-box success"><h4><?php echo wp_kses_post($success); ?></h4><?php if (!empty($members_my_properties_page)) { echo '<a href="'.esc_url($members_my_properties_page).'">'. esc_html__('View your properties.', 'rype-add-ons').'</a>'; } ?></div>
	    <?php } ?>
	    ...
	    <form class="multi-page-form" method="post" action="<?php echo get_the_permalink().$form_action; ?>" enctype="multipart/form-data">
	    	...
	    	<div class="multi-page-form-progress">
				<a href="#general-info" class="multi-page-form-progress-item active"><span class="progress-item-num">1</span> <span class="progress-item-text"><?php esc_html_e('General Info', 'rype-add-ons'); ?></span></a>
				<a href="#property-images" class="multi-page-form-progress-item"><span class="progress-item-num">2</span> <span class="progress-item-text"><?php esc_html_e('Property Images', 'rype-add-ons'); ?></span></a>
	            <a href="#map" onclick="refreshMap()" class="multi-page-form-progress-item"><span class="progress-item-num">4</span> <span class="progress-item-text"><?php esc_html_e('Map', 'rype-add-ons'); ?></span></a>
				<a href="#owner-info" class="multi-page-form-progress-item"><span class="progress-item-num">5</span> <span class="progress-item-text"><?php esc_html_e('Owner Info', 'rype-add-ons'); ?></span></a>
			</div>

			<div class="multi-page-form-content active" id="general-info">
		    	<h3><?php esc_html_e('General Info', 'rype-add-ons'); ?></h3>
				...
				<div class="col-lg-6 col-md-6">
					...
					<div class="row form-block-property-price">
						...
						<div class="col-lg-6 col-md-6 form-block">
	                           <label><?php esc_html_e('Price*', 'rype-add-ons'); ?></label>
						...
						<div class="col-lg-6 col-md-6 form-block">
	                           <label><?php esc_html_e('Price Postfix', 'rype-add-ons'); ?></label>
					...
					<div class="form-block form-block-property-beds">
	                    <label><?php esc_html_e('Bedrooms', 'rype-add-ons'); ?></label>
					...
					<div class="form-block form-block-property-baths">
	                    <label><?php esc_html_e('Bathrooms', 'rype-add-ons'); ?></label>
					...
					<div class="form-block form-block-property-garages">
	                    <label><?php esc_html_e('Garages', 'rype-add-ons'); ?></label>
					...
					<div class="form-block form-block-property-video-img">
                        <label><?php esc_html_e('Video Cover Image', 'rype-add-ons'); ?></label>
				...
				<div class="col-lg-6 col-md-6">
					<div class="form-block form-block-property-address">
                        <label><?php esc_html_e('Street Address*', 'rype-add-ons'); ?></label>
                    ...
                    <div class="form-block form-block-property-location border">
                        <label for="property-location"><?php esc_html_e('Property Location', 'rype-add-ons'); ?></label>
                        <select data-placeholder="<?php esc_html_e('Select a location...', 'rype-add-ons'); ?>" name="property_location[]" id="property-location" multiple>
                            <?php
                        ...
                        <div class="property-location-new">
                            <span class="property-location-new-toggle"><?php esc_html_e("Don't see your location?", 'rype-add-ons'); ?> <a href="#"><?php esc_html_e('Add a new one.', 'rype-add-ons'); ?></a></span>
                            <div class="property-location-new-content show-none">
                                <input class="border" type="text" placeholder="Location name" />
                                <a href="#" class="button"><?php echo rypecore_get_icon($icon_set, 'plus', 'plus'); ?> <?php esc_html_e('Add', 'rype-add-ons'); ?></a>
                                <div class="clear"></div>
                            </div>
                        </div>
                        <?php } ?>
		            ...
		            <div class="form-block form-block-property-amenities border">
                        <label for="property-amenities"><?php esc_html_e('Amenities', 'rype-add-ons'); ?></label>
                        <select data-placeholder="<?php esc_html_e('Select an amenity...', 'rype-add-ons'); ?>" name="property_amenities[]" id="property-amenities" multiple>
                            <?php
                        ...
                        <div class="property-location-new">
                            <span class="property-location-new-toggle"><?php esc_html_e("Don't see your amenity?", 'rype-add-ons'); ?> <a href="#"><?php esc_html_e('Add a new one.', 'rype-add-ons'); ?></a></span>
                            <div class="property-location-new-content show-none">
                                <input class="border" type="text" placeholder="Location name" />
                                <a href="#" class="button"><?php echo rypecore_get_icon($icon_set, 'plus', 'plus'); ?> <?php esc_html_e('Add', 'rype-add-ons'); ?></a>
                                <div class="clear"></div>
		            ...
		            <div class="form-block form-block-property-type border">
                        <label for="property-type"><?php esc_html_e('Property Type', 'rype-add-ons'); ?></label>
		                <select name="property_type" id="property-type">
		                    <option value=""><?php esc_html_e('Select a property type...', 'rype-add-ons'); ?></option>
		                    <?php 
					...
					<div class="form-block form-block-property-status border">
                        <label for="contract-type"><?php esc_html_e('Contract Type', 'rype-add-ons'); ?></label>
		                <select name="contract_type" id="contract-type">
		                    <option value=""><?php esc_html_e('Select a contract type...', 'rype-add-ons'); ?></option>
		                    <?php 
				...
				<div class="module-border form-block-property-floor-plans">
	                <h3><?php esc_html_e('Floor Plans', 'rype-add-ons'); ?></h3>
	                <div class="form-block property-floor-plans">
	                    <div class="accordion">
	                        ...
	                        if(!empty($edit_floor_plans)) { 
	                            ...
	                            foreach ($edit_floor_plans as $floor_plan) { ?>
	                                <h3 class="accordion-tab"><span class="floor-plan-title-mirror"><?php echo $floor_plan['title']; ?></span> <span class="delete-floor-plan right"><i class="fa fa-trash"></i> <?php esc_html_e('Delete', 'rype-add-ons'); ?></span></h3>
	                                <div class="floor-plan-item"> 
	                                    <div class="floor-plan-left"> 
	                                        <label><?php esc_html_e('Title:', 'rype-add-ons'); ?> </label> <input class="border floor-plan-title" type="text" name="rypecore_floor_plans[<?php echo $count; ?>][title]" placeholder="<?php esc_html_e('New Floor Plan', 'rype-add-ons'); ?>" value="<?php echo $floor_plan['title']; ?>" /><br/>
	                                        <label><?php esc_html_e('Size:', 'rype-add-ons'); ?> </label> <input class="border" type="text" name="rypecore_floor_plans[<?php echo $count; ?>][size]" value="<?php echo $floor_plan['size']; ?>" /><br/>
	                                        <label><?php esc_html_e('Rooms:', 'rype-add-ons'); ?> </label> <input class="border" type="number" name="rypecore_floor_plans[<?php echo $count; ?>][rooms]" value="<?php echo $floor_plan['rooms']; ?>" /><br/>
	                                        <label><?php esc_html_e('Bathrooms:', 'rype-add-ons'); ?> </label> <input class="border" type="number" name="rypecore_floor_plans[<?php echo $count; ?>][baths]" value="<?php echo $floor_plan['baths']; ?>" /><br/>
	                                    </div>
	                                    <div class="floor-plan-right">
	                                        ...
	                                        <div>
	                                            ...
	                                            <span><em><?php esc_html_e('Provide the absolute url to a hosted image.', 'rype-add-ons'); ?></em></span>
	                                        </div>
	                                    </div>
	                                    <div class="clear"></div>
	                                </div> 
	                    ...
	                    <span class="button light small add-floor-plan"><i class="fa fa-plus"></i> <?php esc_html_e('Create New Floor Plan', 'rype-add-ons'); ?></span>
	                </div>
	            </div>
                ...
                if(!empty($custom_fields)) { ?>
                    ...
                    <h3><?php esc_html_e('Additional Details', 'rype-add-ons'); ?></h3>
                    <?php $count = 0;
                    echo '<div class="row">';                    
                ...
                <!-- hook in for add-ons -->
                <?php do_action('rao_real_estate_after_property_submit_general', $edit_property_id); ?>

			</div><!-- end general info -->

			<div class="multi-page-form-content" id="property-images">
				<h3><?php esc_html_e('Property Images', 'rype-add-ons'); ?></h3>
				...
				<div class="form-block featured-img">
	                ...
	                <label for="featured_img"><?php esc_html_e('Featured Image', 'rype-add-ons'); ?></label><br/>
	                <input id="featured_img" name="featured_img" type="file">
	            </div>

	            <div class="form-block gallery">
	            	<label><?php esc_html_e('Gallery Images', 'rype-add-ons'); ?></label>
	            	...
	            	<div class="additional-img-container">
		            		...
		            		if(!empty($edit_additional_images)) {
		            			foreach ($edit_additional_images as $edit_additional_image) {
		            				if(!empty($edit_additional_image)) {
			            				$additional_img_attachment_id = rao_get_attachment_id_by_url($edit_additional_image); ?>
			            				<table>
			            					<tr>
			            					<td>
			            					<div class="media-uploader-additional-img">
		                        			<img class="additional-img-preview" src="<?php echo $edit_additional_image; ?>" alt="" />
		                        			<a href="<?php echo get_the_permalink().'?edit_property='.$edit_property_id.'&additional_img_attachment_id='.$additional_img_attachment_id; ?>" class="delete-additional-img right"><i class="fa fa-trash"></i> <?php esc_html_e('Delete', 'rype-add-ons'); ?></a>
		                        			</div>
			            					</td>
			            					</tr>
			            				</table>
	            		...
	            		<table>
	                        ...
	                        <span class="delete-additional-img right"><i class="fa fa-trash"></i> <?php esc_html_e('Delete', 'rype-add-ons'); ?></span>
	                        </div>
	                        </td>
	                        </tr>
	                    </table>
	                </div>
	                <span class="button light small add-additional-img"><i class="fa fa-plus"></i>  <?php esc_html_e('Add Image', 'rype-add-ons'); ?></span>

	            </div>
			</div><!-- end property images -->

			<div class="multi-page-form-content" id="map">
            	<h3><?php esc_html_e('Map', 'rype-add-ons'); ?></h3>
            	<div class="left">
                    <div class="row">
                        <div class="col-lg-6 col-md-6 form-block">
                            <input type="text" class="border" name="latitude" id="property_latitude" placeholder="<?php esc_html_e('Latitude', 'rype-add-ons'); ?>" value="<?php if(isset($latitude)) { echo $latitude; } else { echo esc_attr($_POST['latitude']); } ?>" />
                        </div>

                        <div class="col-lg-6 col-md-6 form-block">
                            <input type="text" class="border" name="longitude" id="property_longitude" placeholder="<?php esc_html_e('Longitude', 'rype-add-ons'); ?>" value="<?php if(isset($longitude)) { echo $longitude; } else { echo esc_attr($_POST['longitude']); } ?>" />
                        </div>
                    </div>
            	</div>
            ...
            <div class="multi-page-form-content" id="owner-info">
	            <h3><?php esc_html_e('Owner Info', 'rype-add-ons'); ?></h3>

	            <div class="form-block form-block-property-agent-display">
	                <label><?php esc_html_e('What do you want displayed for the agent info?', 'rype-add-ons'); ?></label><br/>
	                <input type="radio" name="agent_display" id="agent_display_author" value="author" <?php if (!empty($_POST)) { if($_POST['agent_display'] == 'author') { echo 'checked'; } } else if(!isset($_POST['agent_display'])) { echo 'checked'; }  ?> /><?php esc_html_e('Your Profile Info', 'rype-add-ons'); ?><br/>
	                <input type="radio" name="agent_display" id="agent_display_agent" value="agent" <?php if(isset($edit_agent_display)) { if($edit_agent_display == 'agent') { echo 'checked'; } } else { if($_POST['agent_display'] == 'agent') { echo 'checked'; } } ?> /><?php esc_html_e('Existing Agent', 'rype-add-ons'); ?><br/>
	                <input type="radio" name="agent_display" id="agent_display_custom" value="custom" <?php if(isset($edit_agent_display)) { if($edit_agent_display == 'custom') { echo 'checked'; } } else { if($_POST['agent_display'] == 'custom') { echo 'checked'; } } ?> /><?php esc_html_e('Custom', 'rype-add-ons'); ?><br/>
	                <input type="radio" name="agent_display" id="agent_display_none" value="none" <?php if(isset($edit_agent_display)) { if($edit_agent_display == 'none') { echo 'checked'; } } else { if($_POST['agent_display'] == 'none') { echo 'checked'; } } ?> /><?php esc_html_e('None', 'rype-add-ons'); ?><br/>
	            </div>
	            <br/>
	            ...
	            <div class="form-block form-block-agent-options form-block-select-agent <?php if(isset($edit_agent_display) && $edit_agent_display == 'agent') { echo 'show'; } else { echo 'show-none'; } ?>">
	                ...
	                ?>
	                <label for="agent_select"><?php esc_html_e('Select Agent', 'rype-add-ons'); ?></label><br/>
	                <div class="form-block border">
	                    <select name="agent_select">
	                        <option value="" placeholder="<?php esc_html_e('Select an option...', 'rype-add-ons'); ?>"></option>
	                    ...
	                    <?php else: ?>
	                        <option value=""><?php esc_html_e('Sorry, no agents have been posted yet.', 'rype-add-ons'); ?></option>
	                        </select>
	                    <?php endif; ?>
	                </div>
	            </div>
	            ...
	            <div class="form-block form-block-agent-options form-block-custom-agent <?php if(isset($edit_agent_display) && $edit_agent_display == 'custom') { echo 'show'; } else { echo 'show-none'; } ?>">
	                <label><?php esc_html_e('Custom Owner/Agent Details', 'rype-add-ons'); ?></label>
	                <input type="text" class="border" name="agent_custom_name" placeholder="<?php esc_html_e('Name', 'rype-add-ons'); ?>" value="<?php if(isset($edit_agent_custom_name)) { echo $edit_agent_custom_name; } else { echo esc_attr($_POST['agent_custom_name']); } ?>" />
	                <input type="text" class="border" name="agent_custom_email" placeholder="<?php esc_html_e('Email', 'rype-add-ons'); ?>" value="<?php if(isset($edit_agent_custom_email)) { echo $edit_agent_custom_email; } else { echo esc_attr($_POST['agent_custom_email']); } ?>" />
	                <input type="text" class="border" name="agent_custom_phone" placeholder="<?php esc_html_e('Phone', 'rype-add-ons'); ?>" value="<?php if(isset($edit_agent_custom_phone)) { echo $edit_agent_custom_phone; } else { echo esc_attr($_POST['agent_custom_phone']); } ?>" />
	                <input type="text" class="border" name="agent_custom_url" placeholder="<?php esc_html_e('Website', 'rype-add-ons'); ?>" value="<?php if(isset($edit_agent_custom_url)) { echo esc_url($edit_agent_custom_url); } else { echo esc_url($_POST['agent_custom_url']); } ?>" />
	            </div>
	        </div><!-- end owner info -->

			<div class="multi-page-form-nav">
				<div class="multi-page-form-nav-item form-prev button show-none" onclick="refreshMap()"><?php esc_html_e('Previous', 'rype-add-ons'); ?></div>
				<div class="multi-page-form-nav-item button disabled" onclick="refreshMap()"><?php esc_html_e('Previous', 'rype-add-ons'); ?></div>
				<div class="multi-page-form-nav-item form-next button right" onclick="refreshMap()"><?php esc_html_e('Next', 'rype-add-ons'); ?></div>
